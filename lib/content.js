...
export function getAllContent(collection) {
  try {
    ...
    if (!fs.existsSync(collectionPath)) {
      console.log(`Collection path does not exist: ${collectionPath}`)
      return []
    }
    
    const fileNames = fs.readdirSync(collectionPath)
    console.log(`Loading ${collection} files:`, fileNames)
    ...
    console.log(`Loaded ${allContent.length} items for ${collection}`)
    return allContent
  } catch (error) {
